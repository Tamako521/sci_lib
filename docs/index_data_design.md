...
```text
AuthorGraph 基于 coauthor_graph.dat 进行各阶完全子图计数
GUI 只显示阶数和数量
```

F6 不在启动时计算，只在用户点击聚团分析时触发。如果完整 DBLP 上仍然过慢，阶段 6 再讨论是否预计算或限制阶数。
...
- 基于 `coauthor_graph.dat` 实现各阶完全子图计数。
- GUI 聚团分析 tab 只显示：
...
处理：F6 不在启动时计算。先基于图索引实现正确功能；如果完整数据上仍不可接受，再在阶段 6 单独讨论预计算、阶数限制或剪枝策略。
